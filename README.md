...
- 재고 저장, 조회, 수량 조절
- 식재료 / 주류 / 센서 실시간 SSE 스트림
- 스캔 요청과 실제 AI/Jetson 인식 결과 전달 분리
- 주류 인식 후 현재 재고 기반 추천 결과 전달 흐름
...
### 내부 입력용 API
아래 API는 Jetson/AI가 백엔드로 인식 결과와 센서 이벤트를 넣을 때 사용하는 내부 입력용 API입니다.
FE는 이 API를 직접 호출하지 않고 SSE와 추천 API를 통해 결과를 받습니다.
...
운영 단계에서 별도 확장할 수 있는 항목은 아래와 같습니다.
...
### 스캔 요청-인식 결과 분리 흐름
1. 프론트가 `POST /api/v1/scan/ingredients/start` 또는 `POST /api/v1/scan/liquor/start` 호출
2. 백엔드가 `accepted`와 `scan_request_id`를 먼저 반환
3. 백엔드는 임시 인식 결과를 자동 발행하지 않음
4. Jetson/AI가 실제 인식 결과를 `/api/v1/recognitions/ingredients` 또는 `/api/v1/recognitions/liquor`로 전송
5. 백엔드가 해당 `scan_request_id`를 포함한 SSE를 발행
6. 주류 인식 결과가 확정되면 추천 로직을 실행하고 `recommendation` SSE를 발행

즉 현재 프론트는 버튼으로 스캔 요청을 시작할 수 있지만, 화면에 표시되는 인식 결과는 실제 AI/Jetson 입력이 들어온 뒤에만 발행됩니다.

### 센서 이벤트 흐름
1. Jetson-Arduino bridge가 `POST /api/v1/sensors/events`로 초음파 거리 센서와 모션감지 센서 판단 결과를 전송
2. 백엔드가 문열림 우선순위에 따라 `ingredient_scan`, `liquor_scan_ready`, `standby` 중 권장 모드를 계산
3. 백엔드가 `GET /api/v1/stream/sensors` 구독자에게 `sensor` SSE를 발행
4. 센서 이벤트만으로 재고 저장이나 추천 생성을 수행하지 않음
5. 실제 식재료/주류 인식 결과는 Jetson/AI가 기존 recognition API로 별도 전송
...
### 6. AI/Jetson/Arduino 연동 smoke test
서버를 실행한 뒤 AI팀원과 센서·인식 payload를 맞출 때 아래 스크립트로 핵심 흐름을 한 번에 확인할 수 있습니다.

```bash
python scripts/smoke_hardware_flow.py --base-url http://127.0.0.1:8000
```

이 스크립트는 아래 순서로 API를 호출합니다.

1. 식재료 스캔 요청 생성
2. 문열림 센서 이벤트 전송 및 `ingredient_scan` 모드 확인
3. 식재료 인식 결과 전송
4. 재고 일괄 저장
5. 주류 스캔 요청 생성
6. 문닫힘+사용자 접근 센서 이벤트 전송 및 `liquor_scan_ready` 모드 확인
7. 주류 인식 결과 전송
8. 현재 재고와 주류 기준 추천 3개 조회

기본값은 `green_onion`과 `soju`이며, 실제 모델 결과에 맞춰 바꿔 실행할 수 있습니다.

```bash
python scripts/smoke_hardware_flow.py --ingredient pork --liquor beer
```
...
Jetson/AI 인식 결과 입력용 API는 Swagger에서 숨겨져 있으며, 센서 이벤트 API는 하드웨어 연동 확인을 위해 문서에서 확인할 수 있습니다.
AI/Jetson 계약은 `docs/api_contract_ai.md`를 기준으로 맞춥니다.
