...
class InventoryItemData(BaseModel):
    ingredient_name: str = Field(description="재고에 저장된 식재료의 영문 표준 이름입니다.")
...
class InventoryBulkCreate(BaseModel):
    items: list[str] = Field(
        ...
        description="최종 저장할 식재료 이름 배열입니다. 같은 이름이 여러 번 들어오면 수량이 그만큼 증가합니다.",
        examples=[["green_onion", "green_onion", "onion"]],
    )

    model_config = {
        "json_schema_extra": {
            "example": {
                "items": ["green_onion", "green_onion", "green_onion", "onion"]
            }
        }
    }
...
class InventoryQuantityPatchRequest(BaseModel):
    ingredient_name: str = Field(
        min_length=1,
        max_length=100,
        description="수량을 조절할 식재료의 영문 표준 이름입니다.",
