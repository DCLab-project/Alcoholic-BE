...
- seed DB에서 내려온 추천은 `recommendation_source: "seed"`입니다.
- Gemini로 부족분을 보완한 추천은 `recommendation_source: "llm_fallback"`입니다.
...
추천 엔진은 검수된 seed DB를 1차 추천 지식으로 사용합니다.
현재 보유 재료와 필터 조건에 맞는 seed 후보가 3개보다 적고 `llm_fallback=true`이면 Gemini API를 호출해 부족한 추천 수만 보완합니다.
생성 결과는 백엔드에서 JSON schema와 도메인 규칙으로 재검증한 뒤 응답에 포함하며, 검증 실패 시 기존 seed 추천 결과만 반환합니다.
